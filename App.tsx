...
import type { Fund, FundBranding } from './data/fundData';
...
const DEFAULT_BRANDING: FundBranding = {
    primaryColor: '#ff8400',
    secondaryColor1: '#003a70',
    secondaryColor2: '#edda26',
    logoUrl: 'https://gateway.pinata.cloud/ipfs/bafkreigagdtmj6mbd7wgrimtl2zh3ygorbcvv3cagofbyespbtfmpn2nqy'
};

function applyBranding(branding: FundBranding) {
    const root = document.documentElement;
    root.style.setProperty('--brand-primary', branding.primaryColor);
    root.style.setProperty('--brand-secondary-1', branding.secondaryColor1);
    root.style.setProperty('--brand-secondary-2', branding.secondaryColor2);
}
...
function App() {
  ...
  const [activeBranding, setActiveBranding] = useState<FundBranding>(DEFAULT_BRANDING);
  ...
  useEffect(() => {
    // Apply theme on initial load from cache or defaults
    try {
        const cachedBranding = sessionStorage.getItem('activeBranding');
        const brandingToApply = cachedBranding ? JSON.parse(cachedBranding) : DEFAULT_BRANDING;
        applyBranding(brandingToApply);
        setActiveBranding(brandingToApply);
    } catch (e) {
        applyBranding(DEFAULT_BRANDING);
        setActiveBranding(DEFAULT_BRANDING);
    }
  }, []);

  useEffect(() => {
    if (activeFund?.branding) {
        applyBranding(activeFund.branding);
        setActiveBranding(activeFund.branding);
        sessionStorage.setItem('activeBranding', JSON.stringify(activeFund.branding));
    } else if (authState.status === 'signedOut') {
        applyBranding(DEFAULT_BRANDING);
        setActiveBranding(DEFAULT_BRANDING);
        sessionStorage.removeItem('activeBranding');
    }
  }, [activeFund, authState.status]);
      ...
      return (
        <div className="flex-1 flex justify-center p-4">
            <div className="w-full max-w-lg px-4 pt-8 sm:pt-12">
                {page === 'register' ? (
                <RegisterPage onRegister={authClient.register} switchToLogin={() => setPage('login')} logoUrl={activeBranding.logoUrl} />
                ) : page === 'forgotPassword' ? (
                <ForgotPasswordPage onSendResetLink={authClient.sendPasswordResetEmail} switchToLogin={() => setPage('login')} logoUrl={activeBranding.logoUrl} />
                ) : (
                <LoginPage onLogin={authClient.signIn} switchToRegister={() => setPage('register')} switchToForgotPassword={() => setPage('forgotPassword')} logoUrl={activeBranding.logoUrl} />
                )}
            </div>
        </div>
      );
  ...
  if (!currentUser) {
      return (
          <div className="bg-[var(--brand-secondary-1)] text-white h-screen font-sans flex flex-col">
  ...
  return (
    <div className="bg-[var(--brand-secondary-1)] text-white h-screen font-sans flex flex-col md:flex-row overflow-hidden">
      <IconDefs />
      <SideNavBar 
        navigate={navigate}
        currentPage={page}
            ...
            {page === 'profile' && (
               <div className="relative flex justify-center items-center my-8">
                  <div className="text-center">
                      <h1 className="text-3xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-[var(--brand-primary)] to-[var(--brand-secondary-2)]">
                        {t('profilePage.title')}
                      </h1>
                      {activeIdentity && (
